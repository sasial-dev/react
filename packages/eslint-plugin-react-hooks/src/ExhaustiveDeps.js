...
export default {
	...
	create(context) {
		...
		function visitFunctionWithDependencies(
			...
			//               ^^^ true for this reference
			// const [binding, setBinding] = useBinding() / React.useBinding()
			//        ^^^      ^^^ true for these references
			...
			function isStableKnownHookValue(resolved) {
				...
				} else if (name === "useBinding") {
					if (
						id.type === "ArrayPattern" &&
						id.elements.length === 2 &&
						isArray(resolved.identifiers)
					) {
						// First tuple value (binding) is stable.
						if (id.elements[0] === resolved.identifiers[0]) {
							const references = resolved.references;
							let writeCount = 0;
							for (let i = 0; i < references.length; i += 1) {
								if (references[i].isWrite()) writeCount += 1;
						...
						// Second tuple value (setter) is stable.
					...
					switch (setStateRecommendation.form) {
						...
						case "updater":
							extraWarning =
								...
								}(${setStateRecommendation.missingDep.slice(0, 1)} => ...)' if you only need '${
